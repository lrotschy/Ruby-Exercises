...
class Phrase
  def initialize(phrase)
    @words = prepare_words_for_count(phrase)
  end

  def prepare_words_for_count(string)
    words = string.downcase.scan(/[a-z'']+'?[a-z'']+|\d+/)
    p words
    words = remove_stray_quotes(words)
    p words
    words = remove_quotes(words)
    p words
  end

  def remove_stray_quotes(array)
    # array.map {|w| w.gsub(/\A'(\w*)\Z/, '\1')}
    array.map { |w| w.gsub(/^'/, '')}
  end

  def remove_quotes(array)
    array.map {|w| w.gsub(/'(\w*)'/, '\1')}
  end
...
p Phrase.new("don't 1 one testing, one,two,three 'car boys' 'quotes' javascript(*&%*&%^&*^)").word_count
