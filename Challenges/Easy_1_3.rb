...
class Octal
  ...
  def to_decimal
    octal = @octal.to_i.digits
    return 0 if octal.any? { |n| n == 8 || n == 9 }
    sum = 0
    (0..(octal.length - 1)).each do |n|
      sum += octal[n] * (8 ** n)
    end
    sum
  end
...
# p 124.digits.map { |n| n ** 2 }
# sum = 0
# call digits on octal to reverse it and convert to array of i
# 0..array.length-1 |n|
# sum += array[n] * (8**n)
# return sum
